...
- **Ziel:** 10 Betriebe pro Tag, maximal persönlich, high-end, automatisiert
- **Ein Befehl:** `pipeline_run.mjs --url X --slug Y` → kompletter Betrieb provisioniert
- **3 Phasen:** Phase 1 (Extract+Decide) → Phase 2 (Video 4 Takes) → Phase 3 (Outreach E-Mail)
- **tenant_config.json = SSOT:** Ein JSON pro Betrieb steuert ALLES (Voice Agent, Wizard, Seed, Video, Outreach)
- **Automatische Extraktion:** Playwright crawlt Website, Zefix verifiziert Handelsregister (UID, Sitz, Status), Google Places holt Rating + Bewertungsanzahl
- **Voice Agent aus Template:** Dörfler AG Gold-Standard → 23 Platzhalter → pro Betrieb in <10s generiert
- **Video = 3 Schichten:** Audio (Founder-Segmente + Lisa TTS + STS-Swap) + Screen (Playwright Samsung/Leitsystem) + PiP (Loom, einmal aufgenommen)
- **Timing-Constraints:** Jedes variable Element hat ein festes Zeitfenster (Silbenzahl/Sekunden) damit PiP synchron bleibt
- **Founder-Aufwand pro Betrieb:** ~2 Min (Video-Hook + Betriebsfrage bestätigen). Kein Text schreiben.
- **Twilio-Nummern:** Erst nach positiver Rückmeldung kaufen (Kosten-Optimierung)
- **Getestet mit:** Wälti & Sohn AG (5.0★), Stark Haustechnik GmbH (4.8★), Leins AG (5.0★)
- **Scripts:** `pipeline_run.mjs`, `crawl_extract.mjs`, `derive_config.mjs`, `generate_voice_agent.mjs`, `provision_from_config.mjs`
- **Docs:** `docs/gtm/pipeline/` (Specs, Templates, Master-Takes)
- **Marktanalyse:** 585 Betriebe Kanton Zürich (search.ch + Zefix + Google + Website-Crawl). Goldenes Segment: 278 Betriebe (≥4.5★ + wenige Reviews). Hochrechnung Deutschschweiz: ~2'457 Betriebe.
- **Pipeline Bible:** `docs/gtm/pipeline/PIPELINE_BIBLE.md` — 12 Kapitel, komplette Referenz für alle AI-Module
- **20 Video-Verbesserungen:** 16✅ Einbau (Lisa Greeting, Mini-Take-Pool, FAKE-ENDSCREEN, tagesfrisches Datum). Founder-Feedback vollständig dokumentiert.
- **Take 2 = 2 Videos:** Notruf-Variante (Betriebe mit 24/7-Notdienst) + Preis-Variante (alle anderen). Pipeline wählt automatisch. Call-Scripts FINAL.
- **Take 2 Screenflow-Pipeline FINAL (21.04.2026):** One-Command Runner `pipeline_screenflow.mjs --slug <slug> --take 2` → seed → status-bar render → produce (Samsung HTML + Leitsystem) → FFmpeg Splice. ~60-90s pro Tenant. Kern-Architektur: Samsung HTML (412×915, CSS word-wrap für lange Namen), scrcpy-App-Open-Clip als authentische Transition (wird durch Remotion-Component ersetzt), Leitsystem via Playwright + Reveal-Overlay, Status-Bar persistent via PNG + FFmpeg vstack, display_phone konstant (+41 44 505 74 21), Zeit-Sync über `_seed_time`. End-to-End getestet mit 4 Betrieben. Output: `docs/gtm/pipeline/06_video_production/screenflows/<slug>/take2_complete.mp4`.
- **Pipeline-Skalierungs-Refactor S1-S4 (Day 23 EOD, 23.04.2026):** Nach Dörfler AG Master-Fertigstellung heute Abend die Skalierungs-Grundlage für 10/Tag gelegt. **S1 Customer-Registry-Fallback** (`src/web/src/lib/customers/fallback.ts` + `registry.ts`): Tenants ohne explizite TS-Datei werden automatisch aus `docs/customers/<slug>/tenant_config.json` als minimale `CustomerSite` geladen. `generateStaticParams()` liest union explicit + alle tenant_configs → kein Rebuild pro neuem Tenant. Funktionen: `buildCustomerFromConfig(slug)` + `listConfigSlugs()`. **S2 Asset-Cache global shared** (`scripts/_ops/_lib/shared_assets.mjs`): bezel, circle-masks (170/200/260/300), content-mask (r46), samsung-nav, sidebar-profile, status-bar-review werden EINMAL in `docs/gtm/pipeline/06_video_production/screenflows/_shared/` generiert. `ensureAllSharedAssets()` idempotent. Nur tenant-spezifische Assets (windows_toast mit Tenant-Name, Recordings) bleiben pro-Tenant. ~5-10s pro Tenant eingespart. **S3 Multi-Tenant Pipeline Runner** (`scripts/_ops/run_pipeline_multi.mjs`): CLI `--slugs=a,b,c --takes=all --parallel=N --speed=normal --qg=true`. Batch-Parallelität N Tenants gleichzeitig pro Take (Default 2). Shared-Asset-Generierung einmal upfront. Auto-QG nach jedem Take. Summary-Report (PASS/FAIL + Zeiten). Geeignet für 10/Tag. **S4 Speed-Flag in `record_take4.mjs`**: neue CLI-Flag `--speed=fast|normal|demo` (Default: normal). SPEED-Multiplikator `{fast:0.5, normal:1.0, demo:1.3}` auf alle `page.waitForTimeout()` via `w()`-Helper. Dev-Runs ~30s schneller, Prod-Runs unverändert. **3-Betrieb-Dry-Run bestanden:** Leins AG, Wälti & Sohn AG, Stark Haustechnik durch Take 2+3+4, 35/35 QG PASS pro Tenant. Dörfler AG (Master) alle Takes final. Take-Zeiten: Take 2 ~60s, Take 3 ~90s, Take 4 ~150s, total ~5min/Tenant. Mit Parallel=2 → 10 Tenants in ~15-20 Min. **Nächster Schritt:** Founder Review Take 1-4 für ALLE 4 Betriebe (Dörfler + Leins + Wälti + Stark) + finaler Feedback-Loop. Parallel Audio+Voice-Layer = Founder-Aufgabe. Referenz: PIPELINE_BIBLE.md §30, Zielarchitektur D95.
- **Take 3+4 Screenflow-Pipeline FINAL (Day 23, 23.04.2026):** Pipeline komplett für Dörfler AG (Masterbetrieb 10/10). **Demo-Time-Architektur (`demo_time.mjs`):** zentrale Zeit-Quelle, compressed Timeline (heute 08:04 Case-Erstellung → morgen 08:00-10:00 Termin), Werktag-Gate für Schweizer Feiertage (rollt automatisch auf nächsten Werktag). **DEMO_NO_DISPATCH env-Flag:** unterdrückt echte SMS (eCall) und Mails (Resend) in allen Versand-Endpoints (cases, review-requests, reminders). Pipeline rendert Toasts + DB-Events, aber keine externen Provider-Calls. **Take 4 Feature-Set:** Akt1 Termin versenden → Phone1 Reminder-SMS (24h vorher) → Akt2 D15 Case-Detail (Erledigt + Bewertungs-Flow) → Phone2 SMS Bewertungslink → Review Mobile redesigned (high-end, 4 Chips + Freitext, Samsung-Browser-Frame mit Tab-Strip + URL-Bar) → Closing Gold (Case mit Gold-Sternen + Rating=5) → Windows-Toast tenant-branded (Navy+Gold). **Neue Script-Helper:** `demo_time.mjs`, `renderSidebarProfile.mjs`, `renderWindowsToast.mjs`, `renderSamsungNav.mjs`, `renderPhoneBezel.mjs` (mit `ensureContentMask` + `ensurePhonePlatter` für saubere Bezel-Corners). **Pipeline-Entry-Scripts:** `pipeline_screenflow.mjs` (Take 2+3), `record_take4.mjs`, `compose_take4_final.mjs`, `insert_take4_lifecycle.mjs`. Verlauf-Events werden pro Take auf demo_time gepatcht. **Nächster Schritt (24.04.):** 3-Betrieb-Dry-Run Lens AG + Wälti + Stark mit Quality-Gates-Automation (`dry_run_qg.mjs` in Bau). Reihenfolge: (1) Take 2 × 3 Betriebe sequential, (2) Take 3+4 Lens AG solo, (3) Take 3+4 Dörfler + Wälti + Stark parallel. Komplette Detail-Architektur in `docs/gtm/pipeline/PIPELINE_BIBLE.md` §15-§23.
