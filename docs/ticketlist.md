...
**Updated:** 2026-06-30 — **✅ MODUL „LEBEN / RUNNING" ERLEDIGT + LIVE (Parallel-Session, von CC gemergt/deployed).** Neue 10. CEO-App-Sektion „Leben" (`/ceo/leben`, Founder-persönlich, `isAdmin`) mit Tab **Running**: Strava-OAuth + Webhook → `life_activities`, Garmin-CSV-Historie-Import (`raw` verlustfrei), Jungfrau-Marathon-Countdown (2026-09-05). DB-Migration `20260630120000_life_running.sql` angewandt; Env (STRAVA_CLIENT_ID/SECRET/WEBHOOK_VERIFY_TOKEN) in Vercel. **Live verifiziert** (20 Aktivitäten, Auto-Import aktiv). PRs #749/#751/#752/#753/#756. Doku: `modules/leben.md` · `runbooks/strava_setup.md` · `env_vars.md` · Bible-Modulkarte. **NEU im BACKLOG (Founder bewusst geparkt):** **Leben Phase 2 = KI-Coach + Jungfrau-Trainingsplan** (interaktiver Chat, Tagesbilanz aus `life_activities`/`raw`); **Phase 3 = Kraftsport + Ernährung** (die „Bald"-Tabs). Vorlauf:

**Updated:** 2026-06-30 — **💡 NEUE IDEE AUFGENOMMEN: Cold-Mail-Audio-Hook (Stern 3, Leila-Hormozi-Inspiration).** Wenn beim Cold Call niemand rangeht → Nicht-erreicht-Mail mit **~20-Sek-Lisa-Audio** (Play-Button): Lisa stellt sich vor + sagt, was sie kann → „wir haben ein Leitsystem **nur für [Betrieb]** gebaut" (dynamisch) → Übergang zum Klick-Button. Produkt **ist** eine Stimme → hören = ehrlichster Beweis („show, don't tell"), personalisiert, neuartig, erklärt „Leitsystem" in 20 Sek. **Gelockte Rahmen:** Audio **nicht im Mail-Body** (Clients strippen `<audio>`/Autoplay) → lebt auf der **Beweis-Seite** `/p/[token]`, Mail teasert mit Play-Button-Bild + 1 Zeile; Audio-Play = Warm-Signal (tracken). **⚠️ Hängt an V8-Stimme:** gescriptetes 20-Sek-Hochdeutsch-Intro = sicherster Fall (kein „Grüezi"/CH-Namen, renderbar bis es sitzt) **+ billigster Test**, ob die Stimme reicht; überzeugt's nicht → PVC-CH-Stimme muss vor lautem Outreach kommen. Optional: mit kurzer **menschlicher Gunnar-Zeile** öffnen (Echtheit). Festgehalten in `HERO_DEMO_SPEC.md` (eigener Abschnitt + Offen-Item 7). **Designen mit Stern 3** (geparkt; aktuell Fokus Stern 6 Cockpit). Vorlauf:

**Updated:** 2026-06-28 — **✅ STERN 2 ABGESCHLOSSEN (Haken dahinter).** Cold Call komplett: Doku gelockt (frage-first §1–§5) **+ im Tool live** (CEO-App Stern-2-Gesprächsbaum `coldCallScript.ts` neu, §1 Wer abhebt · §2 Opener · §3 Aufsetzer/4 Auffahrten/Killer · §4 CTA · §5 Exits + Drill auf §5; gemerged `#743`/`c85e40a`, Vercel deployt) **+ getrackt**. Verifiziert: tsc + eslint grün, 24 Knoten/0 tote Links/alle erreichbar. Nebenbei: CEO-Journey Go-Feld als schmaler Klapp-Balken (`#742`/`a7ef411`) → Stern-Karten sofort sichtbar. **OFFEN = nur noch Stern 3** (Hero/Demo): **WARM-Opener** (Strang b, knüpft an §4-Folge-Anruf), **Knoten-Set**, **Seed-Demo-Fall**. Vorlauf:

**Updated:** 2026-06-27 — **Stern 3 Hero Cold-Strang komplett (0–~85) + Stern 2 Cold Call frage-first neu (Kern gelockt).** ERLEDIGT: **(1) Hero-Schluss 49–~85** (`HERO_DEMO_SPEC.md`, `9aca3d3`) — 4 Beats (echt+alle-Kanäle → einfach+risikofrei → Warum → Versprechen) + Knoten-Reveal ~82s (Peek-Regel, Zweifel-Labels), kein gesprochenes CTA, kein „Gunnar" laut. **(2) Cold Call neu** (`cold_call_script.md`, `074def3`) — ersetzt alte send-first/13-Min-Fassung; §2 Opener · §3 Discovery (Aufsetzer→4 Auffahrten→Killer) · §4 Übergang/CTA **gelockt** (Selektivität-Privileg, souverän-assumptiv mit Beat + „dann haben Sie's", Folge-Anruf = WARM-Brücke, nur Mail); Haltung/Lieferung dokumentiert. Drift-Schutz: `cold_opener.md` abgelöst-markiert, Stern-2-Karte aktualisiert. **COLD CALL KOMPLETT (Forts. 2026-06-27):** **§1 „Wer abhebt"** + **§5 Exits** gelockt → Stern 2 §1–§5 end-to-end fertig (`bcfa145`++). Haltung: Ehrlichkeit statt Technik (Hormozi), Nein sofort ehren/kein Drehen, zeigen-statt-sagen (keine „verkauf nichts"-Negation). §1-Kernstrang „Worum geht's?" = Entlastung für die Partnerin („Tagesstress" vorwurfsfrei, „Rücken frei"). **OFFEN (nächste Runde):** **WARM-Opener** (Strang b, knüpft an §4-Folge-Anruf an); **Knoten-Set**; **Seed-Demo-Fall** umstellen. **BACKLOG NEU:** `opener_anker` = 4. Enrich-Feld (faktisch/prüfbar, nie Lob — Gründungsjahr/Notdienst/Generation) fürs Opener-Lob, **auf Eis**; **4–15 MA = Mittelbetrieb = eigener Customer Journey** (separat). Vorlauf:

**Updated:** 2026-06-26 — **Stern 3: Hero-Demo-Definition gestartet (Cold-Strang 0–49 gelockt).** Neue Beweis-Seite = Hero 90s + Knoten statt 4 Videos (~13 Min). Heute **0–49 sekundengenau** festgezurrt (Script + Screenflow) → neu `gtm/pipeline/HERO_DEMO_SPEC.md` (WAS), von Stern-3-Karte verlinkt (WARUM = demo-architektur, WIE = PIPELINE_BIBLE). **2 Anliegen-Varianten** (Sani „Bad" / Wärme „Wärmepumpe"; GT = Wärme-Cluster, datenbelegt MS+Regiotherm), fixe Swap-Slots (driftfrei), Demo-Adresse Frauenfeld (**Thurgau-gebunden**), Karte-danach = **echte** Leitzentrale-Interaktion (NEU-Filter → Fall → VERLAUF). Ton Hormozi/CH, revDSG-Disclosure, Hochdeutsch. **Live nebenbei:** Dörfler `boosted_keywords` publiziert (#741, retell-publish, is_published ✓). **OFFEN (morgen):** Hero-Schluss 49–90, WARM-Opener, Knoten-Set, Seed-Demo-Fall auf neuen Anliegen umstellen, Demo-Adresse pro Kanton, Text-im-Fall = Pipeline-Bible-Neubau. Vorlauf:

**Updated:** 2026-06-25 — **Stern 1 sauber abgeschlossen + auf Stern-3-Doku-Standard gebracht.** ERLEDIGT: **S1-Härtung** (autoritativer `kanton` + Migration `20260625…_leads_kanton`; Branche-Filter `_geo_icp`; Anrede `Herr/Frau Nachname` `_anrede`/`enrich_new_leads`; Größen-Tabs vollständig #736; **Vollerfassung + Anreichern** im Bedienpult #737). **Doku-Pass:** neu `architecture/contracts/leads_contract.md` (Schema-SSOT, Hoheits-Regel Daten/Status, ICP-Gate, `kanton`, `journey_events`) + `gtm/sales/STERN_1_RUNBOOK.md` (Go/Vollerfassung/Anreichern/Gegenprüfung/Purge); **Reconcile** `stern_1_kontakt` + Journey-Bible §3 + INDEX von der CSV-Ära → DB-Pipeline (CSV-Motor `build_leads`/`leads.csv` = Legacy). **Thurgau-Vollerfassung praktisch komplett** (2 Läufe): #28174494353 **1196** + #28179983924 **+90** (lief vor Budget leer = Sättigung) → **~1286** geparkt, 0 Fehler, nur TG. **Pagination-Fix** (Kontaktliste schnitt still bei 1000 ab). **GEMERGED:** Doku-Pass + Pagination-Fix + Region-Purge-Modus → **PR #738 in `main`** (squash `ffc3c30`, lint+build grün, Vercel deployt → volle ~1286 sichtbar). **STERN 1 = GRUNDLAGE STEHT, ABGEHAKT — ruht.** **NEU OFFEN (wenn dran):** Thurgau anreichern (Größe/Inhaber) → Gegenprüfung; Falsch-Region-/Alt-Crawl-Purge (`purge.yml mode=region/stale`, Dry-Run zuerst); dann Stern 2. Vorlauf:

**Updated:** 2026-06-23 EOD — **Handy = Laptop: Mobil-Parität + „Go"-Discovery live.** ERLEDIGT: Secrets-Spiegelung Vercel→GH Actions + `GH_DISPATCH_TOKEN` + `GH_ISSUES_TOKEN`-Rename (#693); Cockpit mobil (Karten/Funnel-Fix/Konsolidierung auf `/ceo/journey`, Prototyp-HTML + `assemble_thurgau` raus, #694); Crawl→DB additiv (`crawl_extract` place_id + `crawl_to_leads`, Match place_id→website→skip, Founder-Felder geschützt, #695); Bedienpult `/api/ceo/ops/dispatch` (admin-gated, Whitelist, #696); **„Go"-Discovery** Kanton+Ort (Deutschschweiz 21 Kt/3'718 Orte aus `swiss-cities`/`language=de`) + Anzahl → `discover.yml`/`discover_to_leads` (additiv, N _neue_/Lauf, #697/#698/#700); Werkstatt entfernt (#699). Sicherheit (Laptop-Review): Slug-+Kanton-Input-Härtung (Actions-Injection), CSRF SameSite-Lax. **NEU OFFEN:** (1) Founder-**End-to-End-Test** Go vom Handy (Token-Check); (2) **425 Alt-Crawl-Leads bereinigen** — `purge_stale_leads.mjs` / `purge.yml`, Dry-Run zuerst (behält aktive Pipeline/gepflegte Leads); (3) Lauf-Status/Ergebnis in der App statt Actions-Log (v2); (4) nächste Knopf-Welle (Lead-Motor/Outreach/Tagesliste); (5) `crawl.yml`/`purge.yml` bleiben ohne UI in der Whitelist (bei Bedarf per Dispatch). Vorlauf: ERLEDIGT: (1) **Morning-Report-Ausfall behoben** — Vercel-Env `GH_DISPATCH_TOKEN` fehlte (MR4-Kette tot, Route 500); Founder hat PAT + Env + Redeploy gesetzt, end-to-end verifiziert → ab morgen wieder pünktlich (05:43 UTC). (2) **`proof_watch`-Tabelle** (MR2, #651) war nie in Prod migriert → Watch-Heartbeat schrieb ins Leere; **Migration live angewandt** (Management-API), 9 Spalten verifiziert. (3) **T1 aus Watch-Report-Signal raus** (#687) — canonical-Rauschen (~1197s bei allen) verfälschte die Lead-Sortierung. (4) **db-migrate (#674) + V10 (#676)** verifiziert abgeschlossen. **NEU OFFEN (niedrig):** `proof_watch_report.mjs` liest noch Bunnys QA-verschmutztes Aggregat statt der neuen `proof_watch`-Tabelle → für saubere Per-Prospect-Watch-Tiefe den Report auf die Tabelle umstellen (sinnvoll ab Welle 2, sobald die Tabelle Daten trägt). Vorlauf:
...
| SAL8 | **DB-Lead-Motor + Doku-Parität** — `leads`-Tabelle = SSOT; `discover_targeted`/`enrich_new_leads`/`crawl_to_leads`; `kanton` autoritativ; Anrede `Herr/Frau Nachname`; Größen-Tabs (#736); Vollerfassung+Anreichern (#737). Doku: `leads_contract.md` + `STERN_1_RUNBOOK.md` + Reconcile (CSV-Ära→DB). | ✅ 25.06. |
| SAL9 | **Thurgau-Vollerfassung anreichern** (~1286 Betriebe geparkt, Größe `?`) → Größe/Inhaber füllen → Founder-Gegenprüfung → cold-call-fähig | OFFEN |
| SAL10 | **425 Alt-Crawl-Leads bereinigen** (`purge.yml`, Dry-Run zuerst — schützt aktive/gepflegte Leads) | OFFEN |
