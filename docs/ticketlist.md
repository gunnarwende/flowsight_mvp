...
| V8 | **Lisa Schweizer Stimme (de-CH) — „aufhören, wie eine Deutsche zu klingen"** (Founder-Bauchweh 22.06.) — Ist-Stand: Engine `de-DE` trotz „Schweizer Hochdeutsch"-Prompt, Custom-ElevenLabs-Stimme (deutsch-deutsch), **kein** Pronunciation-Dictionary. **Sofort:** Engine → de-CH (falls Retell-Support) · Pronunciation-Dictionary pro Tenant (Helvetismen *Lavabo/Velo/Spital/parkieren* + Einzugsgebiet-Ortsnamen) · Helvetismen-Glossar im Prompt. **Mittel:** Schweizer-Hochdeutsch-Stimme wählen/klonen (CH-Quelle). **Später/parken:** echtes Mundart (evtl. Hybrid Mundart-Begrüssung). Reframe: Lisa = Fallback → Messlatte „besser als Combox", nicht Mundart-perfekt. Prüfstein **BigBen** (Zürichsee). Detail: `docs/_strategy_notes/2026-06-22_voice-ch-und-adresse.md`. **Live-Agent nicht ohne Founder-Go.** | hoch | OFFEN |
| V9 | **Adresse: Verteidigung in der Tiefe (gegen Fehlfahrt)** (Founder-Bauchweh 22.06., Schichten 2/3/4 bestätigt) — heute kein Live-Read-back (V3, wegen Stimm-Aussprache) → einziges Netz = Post-Call-SMS. Schichten: (1) **PLZ-Anker** → PLZ→Ort via amtl. Post-Verzeichnis *(Grundlage für 2)*; (2) **Strasse gegen echte Strassen dieser PLZ** prüfen (Swiss-Post-Match-API) ✅; (3) **Read-back wieder ein** sobald Stimme die Namen sauber sagt (hebt V3 gezielt auf) ✅; (4) **Inhaber-Augen + Karten-Pin** in der Leitzentrale, Lisa stellt nie selbst zu ✅; (5) Rückrufnummer = Backstop *(gratis)*; (6) SMS-Korrektur *(gratis, spätere Schicht)*. Hängt an V8 (Read-back braucht saubere Aussprache). Verkaufs-Reframe: „Sie entscheiden — kein Techniker fährt blind los." Detail: `docs/_strategy_notes/2026-06-22_voice-ch-und-adresse.md`. | hoch | OFFEN |
