...
**Updated:** 2026-06-09 — **🚀🚀 ERSTER ECHTER OUTREACH RAUS — Dörfler AG (Meilenstein).** Live-Mail an `info@doerflerag.ch`, Absender Gunnar Wende, Reply-To→Outlook, Proof-Link `8a0fbf…`, Resend `53aba1fb`. Davor 3 Founder-Review-Fixes (#596 + Working-Tree): ① Proof-Anrede „Spengler/Heizung" raus (`deriveSalutation` gefixt → „Herr Dörfler"); ② T2-Pixelation = Bunny-Streaming (Quelle CRF-20-sauber) → T2 neu hochgeladen, Founder „top"; ③ T3 „Neuer Fall"-Modal öffnete vor dem Cursor → 6,7s-Dashboard-Dwell-Fix + **neues QG `G_T3_MODAL_NOT_EARLY`** (in `build_take3_final` + BIBLE). Alle 4 Takes frisch auf **09.06** neu gebaut + QG-grün, Proof-Seite aktualisiert. **NEU LIVE (#596): First-View-Alert** — öffnet ein Prospect die Beweis-Seite, kommt automatisch eine E-Mail an den Founder (`sendProofViewAlert`). **Outreach-Tracking** in `docs/sales/pipeline.md` (Outreach-Log + Kadenz Tag-0/3/6-7, Dörfler erfasst). **Pipeline-Härtung (uncommitted im Working-Tree, Founder committet):** Recorder ehren jetzt `APP_URL` (5 Dateien), T3-Dwell, `qg_t3_modal_timing.mjs`. **OFFEN/TODO:** (a) **Reminder-Cron** (Tag-3/6-7 fällige Touches → Founder-Mail) — empfohlen, noch nicht gebaut; (b) **Voice-Sales-Lisa für Outreach-Phase hochziehen** (heute = minimaler Interest-Capture; gewünscht: psychologisch/sales-stark, Aha-Moment, Onboarding-Verweis) + verifizieren dass sie auf 044 552 09 19 published+geroutet ist; (c) FlowSight-Devserver lief für die Pipeline auf :3001 (Retool belegt :3000). VA1-3 (Voice pre-go-live) weiter offen.

**Updated:** 2026-06-08 — **🚀 COCKPIT R8 LIVE (Leitzentrale-Feedback, #593, main@ff39594)** — Founder-Sprachnachricht „Feedback Layer Leitzentrale" autonom abgearbeitet (15 Tasks), tsc+Build grün, **auf prod per Playwright sichtgeprüft**. **2 kritische Wurzelfunde gefixt:** (a) Submit-Endpoint prüfte noch R7-entferntes `wizard.distribution` → Freischalt-Versand war komplett blockiert; (b) „per E-Mail melden"-Toggle war Schein-Schalter (Webhook ignorierte ihn). **Kern (L-17, Founder-Präferenz):** Gold-Stern-Sperre an der Wurzel — Sterne/Strang werden nur gold, wenn Pflichtfelder sitzen, sonst Inline-Hinweis „fehlt noch X". + L-1 Pill-Back · L-2 ◆→🤍 · L-3/L-6 Stern-Notizen · L-4/L-8 Demo-Texte raus · L-5 Dropdown lesbar · L-11 Place-ID-Hilfe · L-13 goldener Leitsystem-Umriss · L-14 Fälle-Card raus · L-16 Dienstleister dezent · L-18 PDF auf Hauptseite. L-15 AVV verifiziert (Entwurf ok, Anwalt offen). Tracker `cockpit_leitzentrale_buildplan.md`. **Edge (Doku):** Altsessions mit vor-Gating gesetzten Sternen können gold + „fehlt noch" zugleich zeigen — bei frischem Onboarding irrelevant.

**Updated:** 2026-06-08 — **🚀 COCKPIT R6 + R7 LIVE** (PR #590 + #591, main@e883999). **R6:** #3 Name-Personalisierung (durchpropagiert + Agent-Prompt), Backend Korb-Wiring + Nachrichten-E-Mail (dormant bis `call_type`), Init-Persistenz, „reagieren" aufgeräumt. **R7 (Feedback-Box, autonom A→B→C):** **A** Website-Strang neu — Punkt-1-Gate „Spielt das Formular eine Rolle?", alte „Haben Sie Website?"-Frage RAUS, Integrations-Karte (Betreuung/ersetzen-ergänzen/wer-kümmert/Agentur); **B** Badge-Fix auf SSOT (offene Fälle + pending Nachrichten → `opsBadge.ts` + `/api/ops/badge-count`, App-Icon app-weit + Nav-Zähler, echter `badgeCount` im Push); **C** per-Stern konkrete Sani-Placeholder + „So meldet sich" minimal. Tracker: `cockpit_round6_buildplan.md`. **Offene Badge-Reste:** silent push (iOS-Limit), per-Eintrag-„neu"-Marker, **Handy-Test (Founder)**.
