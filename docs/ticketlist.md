...
| V10 | **Retell-Deprecation-Mails (22.06.)** — (1) *Legacy publish endpoint* `POST /publish-agent/` (34 Uses): Quelle war `sync_cockpit_test_agent.mjs` → `client.agent.publish()` aus **retell-sdk@5.2.0** (alte Methode trifft den bare Endpoint). **GEFIXT** → raw `POST /publish-agent-version/{id}` (wie Live-Scripts). **Verify (Founder):** einmal `node --env-file=src/web/.env.local scripts/_ops/sync_cockpit_test_agent.mjs` laufen → in Retell darf kein `/publish-agent/` mehr auftauchen. (2) *Phone-field* `inbound_agent_id` (1 Use, Deadline 03-31 schon vorbei): Live-Routing nutzt längst `inbound_agents[]` — die 1 Nutzung ist vermutlich Altlast/manuell → beobachten. **OFFEN:** per eingeloggter Retell-Doku (`07-20_agent_version_endpoints`) bestätigen, ob `/publish-agent-version/` (= unser Live-Publish, BigBen-Daily) **ebenfalls** abgelöst wird; falls ja → `retell_sync.mjs` + `bigben_publish.mjs` nachziehen. **Live-Agents (BigBen/Dörfler) NICHT ohne Founder-Go + Publish-Regel.** | mittel | TEILWEISE GEFIXT |
