...
**Status:** v4.3 — 87 Decisions (D1-D87). Vorstellungsmaschine v2 (D87): Scripts gefroren, Hybrid-Screenflow, parametrisch skalierbar. Pub/Gastro-Modul (D85): Events, Reservierungen, No-Show, Walk-in — eigene Domäne. Voice→Reservation Polling (D86): sync-calls API bei Webhook-Ausfall. BigBen Pub = erster zahlender Kunde.
...
| D87 | **Vorstellungsmaschine v2.** Scripts gefroren (speakflow_template.md). 4 Takes pro Betrieb, parametrisch skalierbar. Hybrid-Screenflow: Samsung Native (Master-Video + ffmpeg Parameter-Swap) + Playwright Video-Recording (echte Web-App). KPIs/Fälle pro Betrieb aus Machine Manifest (Scout + Google Crawl), NICHT hardcoded. Alte Video-Assets OBSOLET — Neuproduktion ab 18.04. Seed muss zum Betriebsprofil passen (Teamgrösse, Kategorien, Einzugsgebiet, Google-★). Machine Manifest und Vorstellungs-Pipeline sind direkt verknüpft. | **IN ARBEIT** 🔨 | Founder + CC | `vorstellungsmaschine_plan.md`, `screenflow_pipeline_architektur.md`, `take2_screenflow_spec.md` |
