...
| D88 | **Gold-Contact-Pipeline V2 — tenant_config.json als SSOT.** Ein Befehl (pipeline_run.mjs) → Crawl + Zefix + Google + Derive → tenant_config.json → Voice Agent + Seed + Provision. Kein Script liest CLI-Args für Betriebsdaten. Alles aus einem JSON. 3 Phasen: Phase 1 (Extract+Decide), Phase 2 (Video), Phase 3 (Outreach). 3 Betriebe getestet (Wälti, Stark, Leins). Scripts: crawl_extract.mjs (Playwright+Zefix+Google), derive_config.mjs, generate_voice_agent.mjs (23 Platzhalter aus Template), provision_from_config.mjs, pipeline_run.mjs (Orchestrator). | **ENTSCHIEDEN** ✅ | Founder + CC | `pipeline_run.mjs`, `crawl_extract.mjs`, `derive_config.mjs` |
| D89 | **Zefix Handelsregister-Verifizierung.** Jeder gecrawlte Betrieb wird gegen das Schweizer Handelsregister (Zefix REST API) geprüft. Offizieller Name, UID (CHE-xxx.xxx.xxx), Sitz, Status (EXISTIEREND). Casing-Logik: Titlecase bevorzugt über ALLCAPS-Registernamen. Verhindert Widmer-Problem (Betrieb existiert nicht). | **ENTSCHIEDEN** ✅ | Founder + CC | `crawl_extract.mjs` |
| D90 | **Video-Timing-Constraints (feste Zeitfenster).** Jedes variable Element im Video hat ein festes Zeitfenster (Silbenzahl/Sekunden). PiP (Loom) wird EINMAL aufgenommen und für ALLE Betriebe wiederverwendet. Betriebsspezifische Frage + Antwort müssen EXAKT gleich lang sein über alle Betriebe. ElevenLabs TTS Pacing-Control für millisekundengenaue Antwort-Dauer. Video = 3 Schichten (Audio + Screen + PiP) synchron. Mini-Take-Pool (10 Fragen) definiert. Founder nimmt Fragen auf. Timing-Constraints für alle variablen Elemente dokumentiert in PIPELINE_BIBLE.md. | **ABGELÖST durch D96** (3-Schichten + Anker statt fixe Zeitfenster) | Founder + CC | Video-Pipeline Phase 2 |
| D91 | **Twilio-Nummern erst nach positiver Rückmeldung.** Keine Nummern im Vorrat. Pipeline funktioniert OHNE Telefonnummer. Erst wenn Betrieb positiv antwortet und testen will → Nummer kaufen. Kosten- und Überblicks-Optimierung. Für Video-Produktion wird der Anruf SIMULIERT (Screen-Recording). | **ENTSCHIEDEN** ✅ | Founder + CC | `provision_from_config.mjs` |
| D92 | **Kein per-Business Text-Personalisierung.** Speakflow-Script ist FINAL und generisch. Personalisierung kommt aus Daten (Firmenname STS-Swap, betriebsspezifische Screens, Kategorien, Brand-Color), NICHT aus Founder-Text. 0 Minuten Founder-Textarbeit pro Betrieb. Betriebsspezifische Frage + Video-Hook + Öffnungszeiten = PFLICHT (Kern des Videos, höchster Proof). prospect.email = auto aus Crawl. | **ENTSCHIEDEN** ✅ | Founder + CC | `speakflow_template.md`, `derive_config.mjs` |
| D93 | **Take 2: 2-Video-Ansatz (Notruf + Preis).** Statt einem Video mit fixen Zeitfenstern: zwei separate Videos mit eigenem Audio→Screenflow→Loom. Pipeline wählt per `call_proof_variante` (C=Notdienst, B=Preis). Call-Scripts FINAL (`minitakes/Take2/final/`). Eliminiert Timing-Constraints komplett. | **ENTSCHIEDEN** ✅ | Founder | PIPELINE_BIBLE.md §6 |
| D94 | **Demo-Time-Architektur + DEMO_NO_DISPATCH (Day 23).** `demo_time.mjs` = zentrale Zeit-Quelle für Pipeline mit compressed Timeline (heute 08:04 Case-Erstellung → morgen 08:00-10:00 Termin). Werktag-Gate (CH-Feiertage inkl. Zürich) rollt automatisch auf nächsten Werktag — kein Wochenend-/Feiertag-Fall auf Pipeline-Screens. `DEMO_NO_DISPATCH=1` env-Flag unterdrückt in allen Versand-Endpoints (cases, review-requests, reminders) echte SMS (eCall) + Mails (Resend). Pipeline rendert UI-Toasts + DB-Events, aber NIEMALS externe Provider-Calls im Demo-Modus. Saubere Trennung Demo-Rendering ↔ Production-Versand. Verlauf-Events werden pro Take auf demo_time gepatcht für zeitliche Kohärenz. | **ENTSCHIEDEN** ✅ | Founder + CC | `scripts/_ops/pipeline/demo_time.mjs`, PIPELINE_BIBLE.md §23, Take 4 Feature-Set |
| D95 | **Pipeline-Skalierungs-Architektur S1-S4 (Day 23 EOD, 23.04.2026).** Vier Bausteine heben die Screenflow-Pipeline von Einzel-Tenant-Betrieb auf 10 Tenants/Tag. **S1 Customer-Registry-Fallback** (`src/web/src/lib/customers/fallback.ts` + `registry.ts`): Tenants ohne explizite TS-Datei werden automatisch aus `docs/customers/<slug>/tenant_config.json` als minimale `CustomerSite` geladen. `generateStaticParams()` liest union explicit + alle tenant_configs → kein Rebuild pro neuem Tenant. Funktionen: `buildCustomerFromConfig(slug)` + `listConfigSlugs()`. **S2 Asset-Cache global shared** (`scripts/_ops/_lib/shared_assets.mjs`): bezel, circle-masks (170/200/260/300), content-mask (r46), samsung-nav, sidebar-profile, status-bar-review werden EINMAL in `docs/gtm/pipeline/06_video_production/screenflows/_shared/` generiert. `ensureAllSharedAssets()` idempotent. Tenant-spezifisch bleiben nur windows_toast (mit Tenant-Name) + Recordings. Einsparung ~5-10s/Tenant. **S3 Multi-Tenant Pipeline Runner** (`scripts/_ops/run_pipeline_multi.mjs`): CLI `--slugs=a,b,c --takes=all --parallel=N --speed=normal --qg=true`. Batch-Parallelität N Tenants gleichzeitig pro Take (Default 2). Shared-Asset-Generierung einmal upfront, Auto-QG nach jedem Take, Summary-Report (PASS/FAIL + Zeiten). **S4 Speed-Flag in `record_take4.mjs`**: CLI `--speed=fast\|normal\|demo` (Default: normal). SPEED-Multiplikator `{fast:0.5, normal:1.0, demo:1.3}` auf alle `page.waitForTimeout()` via `w()`-Helper. Dev-Runs ~30s schneller, Prod-Runs unverändert. **Dry-Run-Proof:** 3 Betriebe (Leins + Wälti + Stark) × 3 Takes, 35/35 QG PASS pro Tenant, ~5min/Tenant. Mit Parallel=2 → 10 Tenants in ~15-20 Min. **Founder-Review Next:** Take 1-4 für alle 4 Betriebe (Dörfler + Leins + Wälti + Stark) + finaler Feedback-Loop, parallel Audio+Voice-Layer Founder-Aufgabe. | **ENTSCHIEDEN** ✅ | Founder + CC | `fallback.ts`, `registry.ts`, `shared_assets.mjs`, `run_pipeline_multi.mjs`, `record_take4.mjs`, PIPELINE_BIBLE.md §30 |
| D96 | **Video-Pipeline (Phase 2) FUNKTIONAL + GESICHERT (03.06.2026).** Die Pipeline ist steuerbar/antriggerbar: aus `tenant_config.json` → vollständiger, gate-geprüfter 4-Take-Satz. **7 Betriebe produziert + founder-abgenommen** (Weinberger, Obrist, Wälti, Walter, Schaub, Marti, Stark → `07_stresstest/abgenommen/`). **Architektur (final):** 3 Schichten = per-Betrieb-Screenflow (Playwright) + universelles locked Audio (Master + per-Tenant-Greeting-Swap `swap_tenant_greeting`, Slot [44,7.0]) + universelle Maus/Loom (Dörfler-Layer). `holdUntilMaster`-Anker gegen Aufnahme-Jitter; deterministische Overlays: canonical-stars (Weinberger-Referenz, Fenster [72,76.3]) + SMS-Thread-Helligkeits-Detektion. Variante C/B aus `call_proof_variante`; `sanitizeBrandColor` → Sanitär-Blau; Seed synct `brand_color → DB primary_color`. **Quality-Gate-System** (`qg_video.mjs`): G_START0, G_HOMESCREEN0, G_T2_PAUSE/BEEP/TAIL/SMS_OPEN, G_GREETING (STT), G_T4_STARSYNC/CASEOPEN/DOUBLESTAR + Daten-Gate G_T3_KPI_NEU — fail-on-broken/pass-on-good, kein Platzieren bei rotem Gate. **Build:** `seed_screenflow_from_config` → `insert_take3_wizard_case` → `compose_take1_hero`/`build_take{2,3,4}_final --with-mouse`, strikt sequenziell. **Bekannter 5%-Rest:** T4-Recording-Timing-Jitter (caseopen ±1s + Part-5/Erledigt unverankert). **Schlüsseldoc:** PIPELINE_BIBLE.md neugefasst (autoritativ + replikations-bereit; Historie in `docs/archive/`). Ersetzt das fixe-Zeitfenster-Modell aus D90. Nächste Phase: E-Mail (Phase 3) + View-Tracking. | **ENTSCHIEDEN** ✅ | Founder + CC | `qg_video.mjs`, `build_take*_final.mjs`, `apply_canonical_stars.mjs`, `swap_tenant_greeting.mjs`, PIPELINE_BIBLE.md (Neufassung), PRs #533-#542 |
| D97 | **Phase 3: E-Mail/Outreach — Beweis-Seite + automatisierter Versand (04.–05.06.2026).** „Mail = Deckel, Seite = Schatz": pro Betrieb private `/p/[token]`-Seite (mobil-first, noindex, navy/gold, Bunny-Stream-Videos + View-Tracking), EIN Link in der Mail. **Versand automatisiert** via `send_outreach.mjs`: High-End-HTML aus „Gunnar Wende <gunnar.wende@flowsight.ch>" (Resend, Reply-To Founder → Antworten in Outlook), Foto inline (cid), Link aus `proof_pages`, NULL Copy-Paste. 🔒 **Test-Default = Founder; echter Versand nur `--live`** (pro Betrieb auf Founder-Wort). **Kein M365-Graph** (Resend bleibt Mail-Provider; Graph nur Kalender, D32). Mail = Hell-Premium-Standard (Sanitär = Hell-Modus, FB36). | **ENTSCHIEDEN** ✅ | Founder + CC | `send_outreach.mjs`, `build_proof_page.mjs`, `app/p/[token]/`, `CTA.md`, `outreach_templates.md` |
| D98 | **Canonical T1 — EIN Founder-Intro für ALLE Betriebe (05.06.2026).** T1 (face-only, kein Text) ist betriebsübergreifend **bit-identisch** (md5-bewiesen: `take1.wav`+`take1_face.mp4` gleich über alle Tenants). Statt per-Betrieb-Generierung/Upload: **EINE Bunny-GUID + EIN canonical Poster**, von jeder `proof_page` referenziert; vom 14-Tage-Cleanup ausgenommen. Spart Zeit/Token/Speicher + senkt Pipeline-Komplexität dauerhaft (DRY). Pipeline erzeugt/lädt T1 nie mehr pro Betrieb. | **ENTSCHIEDEN** ✅ | Founder + CC | `_lib/bunny.mjs` (CANONICAL_T1_GUID), `build_proof_page.mjs`, `proof_add_variants.mjs`, `expire_proof_pages.mjs`, PRs #566/#567 |
| D99 | **B-Vorlage (Kalt-Outreach, 99 % der Kunden): Struktur fix, Haken variabel (05.06.2026).** Feste 4-Beat-Struktur (Haken → unsichtbarer Preis → Beweis → No-oriented-Schluss); der **Haken** wird per Profil-Signal gewählt — **Archetyp A** (Erreichbarkeit/„verpasster Anruf", klein/inhabergeführt), **B** (Notdienst/„7/24-Versprechen"), **C** (Bewertungs-Lücke bei schwachem Rating) + Slot-Variablen **Geo** (Nachbar/Region) & **Gewerk-Bild**. Prinzipien: Bedrohung senken vor Spannung; Spezifik (echtes Detail) schlägt Hochglanz; Hypothese→messen (A/B via Watch-Funnel). Anker = ICP (inhabergeführt, klein-mittel). | **ENTSCHIEDEN** ✅ | Founder + CC | `CTA.md`, `outreach_templates.md`, `docs/customers/walter-leuthold/outreach/` |
| D100 | **Outreach-Copy-Schritt + Inhaber-Anreicherung in der Pipeline — GEPLANT (Montag 08.06.2026).** Neuer Pipeline-Schritt liest crawlbare Signale aus `tenant_config` (Größe/Erreichbarkeit, Notdienst C/B, Rating+Anzahl, Geo, Gewerk) → wählt Archetyp → füllt Slots → schreibt `email.json` (deterministisch, founder-gated, A/B-gelernt). Plus **Inhaber-E-Mail-Anreicherung** (Impressum/Zefix/LinkedIn statt `info@`). **Noch NICHT verankert** — offener Punkt, einzubauen ohne Qualitätsverlust. | **GEPLANT** 🔜 | Founder + CC | Montags-Paket; STATUS 05.06., `project_doerfler_monday_rerun` |
| D101 | **Onboarding = Self-Service-Cockpit + Premium-Pricing-Pivot (06.06.2026).** Onboarding komplett neu aufgesetzt (alt → `docs/archive/onboarding/Onboarding_bible_v0.md`): **Co-Pilot-Cockpit** statt founder-geführtem Vor-Ort-Termin („Sie bauen Ihr System, wir führen Sie"; IKEA/Self-Checkout). **4 Phasen:** 1 Das Gespräch (Live-Playbook, Mom-Test) · 2 Cockpit (Kunde baut, klick-getrieben, confirm-not-create, ~70 % aus `tenant_config` vorbefüllt) · 3 Review & Go-live (Founder-Review + Zahlung) · 4 Validierung. **Seam:** Onboarding startet beim ersten menschlichen Kontakt (nicht „Ja"); `tenant_config.json` = Through-Line → Cockpit befüllt vor → schreibt Live-Config in DB (`tenants.modules`+`staff`) + Retell-Prompt. **Voice:** 7 Dispositionen → 3 Körbe (Fall/Nachricht/nichts); Routing-Fundament `tenant_callbacks` (generalisiert aus BigBen `pub_callback_requests`). **Pricing (löst D35-Tiers für Akquise ab):** Premium — einmalige Aktivierung + Premium-Monat, **monatlich kündbar = Risiko-Umkehr, kein Trial**, zahlend am Go-live. 0 Kunden → Richtwerte (Aktiv. ~1.5-2.5k / Mo ~1.9-2.4k), erste 3-5 = Preis-Findung, nie rabattieren (Scarcity 10/Mo). **Compliance:** revDSG (nicht primär DSGVO), No-Gos = gesperrte Cockpit-Leitplanken, AVV als Commitment-Artefakt. Build-Backlog OC1-OC7 in `ticketlist.md`. **Build-Stand (08.06.):** OC1-OC6 GEBAUT & LIVE auf `flowsight.ch/aufbau/[token]` (Daten-Layer, alle 3 Stränge, Promote, M2/M3); in 3 Feedback-Runden gehärtet (R6 #590 Name+Korb-Wiring · R7 #591 Website-Umbau+Badge-SSOT · R8 #593 Validierung-an-der-Wurzel + 2 Wurzel-Bugs). OFFEN: OC7 Mail-Versand, Voice-`call_type`-Aktivierung (Korb-Routing dormant bis dahin), 🔴 Voice VA1-3 (pre-go-live). | **ENTSCHIEDEN** ✅ · **Build OC1-6 LIVE** · OC7+Voice-`call_type` offen | Founder + CC | `docs/gtm/onboarding/` (Bible §8 + Playbook + Specs + `cockpit_leitzentrale_buildplan.md`), Memory `project_cockpit_round6_resume` |
| D102 | **Customer Journey = ein konsolidiertes SSOT (09.06.2026).** Aus dem ChatGPT-Entwurf (Long+Short) wurde nach CC-Review *eine* business-echte Strecke: **(1)** Journey beginnt bei der **Beweis-Seite** (Outreach → `/p/[token]`-Video → First-View-Alert als Warm-Call-Trigger → Sales-Lisa-Inbound), nicht erst beim Discovery-Call — das personalisierte Video ist der Aha, nicht der Pitch. **(2)** Discovery **warm** (Video gesehen → 2-3 Anker-Fragen) vs. **kalt** (volle 3-Ebenen) getrennt; Mom-Test als Werkzeug, nicht Liturgie. **(3)** Preis konsistent zum **Premium-Pivot (D101)** — Aktivierung + ~1.9-2.4k/Mo, Founder-Phase als Nähe+Preis-Lock *ohne* Rabatt (Bible „nie rabattieren"). **(4)** Go-live 2-stufig, **Telefonweiterleitung = Kunden-Aktion**; revDSG-Einwand + Wochen-Rapport-als-Churn-Versicherung ergänzt. **Soft-Price-Frame vor Cockpit = bewusst nur Hypothese** (durch echte Gespräche validieren). Alte Long/Short → Archiv. | **ENTSCHIEDEN** ✅ | Founder + CC | `docs/gtm/onboarding/FlowSight_Customer_Journey_SSOT.md` (+ `_Short.md` Gesprächskarte, `_Review_CC.md` Begründung) |
...
| **Pipeline Bible** | **docs/gtm/pipeline/PIPELINE_BIBLE.md** | **Gold-Contact-Pipeline komplette Referenz. §15 Take 2 Screenflow-Architektur. §16 Day-22 Plan Take 3+4.** |
