...
- ✅ **`produce_hero.mjs` + `produce-hero.yml` LIVE (gemerged #773), vom Handy gefahren.** Phase „call" erzeugt die **Lisa+Brunner-Call-Audio** via ElevenLabs (Run #2: 28.8s, Gates PASS, Artefakt `hero-call`). Mobile-Ops-Katalog-Eintrag + Secrets gesetzt.
  - **⚠️ OFFEN (Founder schuldet):** eigene **Brunner-Stimme** (männl. DE ElevenLabs-Voice-ID) — Platzhalter `gunnar` klingt nach Founder. Founder → Voice-ID → CC trägt `brunner` in `eleven.mjs` ein + re-run `upload=true`.
- **Noch NICHT gebaut (Phase 2):** Seed Brunner/Frauenfeld (Supabase) · Screen-Renders (Telefon braucht Gesichts-Footage · Leitzentrale-NEU→Filter→Fall→Verlauf) · **Knoten-Reveal-Frontend + Beweis-Seite „1 Hero + Knoten"** (⭐ vom Handy machbar, kein Founder/Laptop nötig) · Hero-QG · finale Montage (Blocker: Gesichts-Footage).
