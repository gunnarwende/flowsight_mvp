...
function buildDeAgent(p) {
  ...
  const agentSwapTool = {
    ...
    description:
      "Transfer the caller to the multilingual (EN/FR/IT) agent. You MUST call this tool (not just announce a transfer) in these situations: (1) caller's message contains language keywords: english, englisch, français, french, italiano, italian, etc. (2) caller's first message is not understandable German — possible ASR drift from a foreign language. (3) caller explicitly requests another language at any point mid-call. (4) caller fails to respond in German after one clarification. Maximum 1 transfer per call.",
